...
interface UnmatchedMoviesProps {
  importId: number | null;
  onImportComplete: () => void;
  setCurrentStep: (step: string) => void;
}

const UnmatchedMovies: React.FC<UnmatchedMoviesProps> = ({ importId, onImportComplete, setCurrentStep }) => {
  ...
  const [selectedSuggestions, setSelectedSuggestions] = useState<Record<string, Suggestion>>({});
  const [ignoredMovies, setIgnoredMovies] = useState<Set<string>>(new Set());
  ...
  const checkImportStatus = async () => {
    try {
      setIsLoading(true);
      const status = await apiService.getImportStatus(String(importId)) as { status: string; unmatchedMovies?: UnmatchedMovie[] };
  ...
  const loadSuggestionsForAllMovies = async (movies: UnmatchedMovie[]) => {
    const suggestionsData: Record<string, Suggestion[]> = {};
    const selectedData: Record<string, Suggestion> = {};

    for (const movie of movies) {
      try {
        const result = await apiService.getMovieSuggestions(String(importId), movie.original_title || movie.title) as { suggestions?: Suggestion[] };
  ...
  const handleContinueProcessing = async () => {
    try {
      ...
      for (const movie of unmatchedMovies) {
        ...
        if (selectedSuggestion && !ignoredMovies.has(movie.title)) {
          ...
          const resolvedMovie = {
            ...
            cast: [] as string[], // Will be filled by backend
          ...
          await apiService.resolveMovie(String(importId), movie.title, resolvedMovie);
        }
      }
      ...
      for (const movieTitle of ignoredMovies) {
        try {
          await apiService.ignoreMovie(String(importId), movieTitle);
        } catch (error) {
  ...
  const handleSearchMovie = (movieTitle: string) => {
  ...
  const performSearch = async (query: string, year: string | null) => {
    if (!query.trim()) return;

    try {
      setIsLoading(true);
      const result = await apiService.getMovieSuggestions(String(importId), query, year) as { suggestions?: Suggestion[] };
      const movieSuggestions = result.suggestions || [];
  ...
  const toggleExpanded = (movieTitle: string) => {
  ...
  const handleIgnoreMovie = (movieTitle: string) => {
  ...
  const handleUndoIgnore = (movieTitle: string) => {
                  ...
                  <div className="suggestions-container">
                    <div className="suggestions-list">
                      {(() => {
                        ...
                        const otherSuggestions = movieSuggestions.filter((_: Suggestion, index: number) => index !== selectedIndex);
