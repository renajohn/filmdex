...
const BookComments: React.FC<BookCommentsProps> = ({ bookId, onCommentsChange }) => {
  ...
  const loadComments = async () => {
    ...
    setLoading(true);
    try {
      const fetchedComments = await bookCommentService.getCommentsByBookId(bookId) as Comment[];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setErrors({ submit: (error as Error).message || 'Failed to save comment' });
    } finally {
      setLoading(false);
    }
  };
