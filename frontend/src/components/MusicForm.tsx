...
interface FormDataState {
  ...
  releaseGroupFirstReleaseDate: string | null;
...
const MusicForm: React.FC<MusicFormProps> = ({ cd = null, pendingPhotos, onSave, onCancel }) => {
  ...
  const [availableCovers, setAvailableCovers] = useState<AvailableCover[]>([]);
  ...
  useEffect(() => {
    if (cd) {
      ...
      if (cd.availableCovers && !cd.id) {
        ...
        const currentCoverIndex = cd.availableCovers.findIndex((c: AvailableCover) => c.url === cd.cover);
      ...
      setFormData({
        ...
        releaseYear: cd.releaseYear as string || '',
        labels: Array.isArray(cd.labels) ? cd.labels : (cd.labels ? [cd.labels as string] : []),
        catalogNumber: cd.catalogNumber || '',
        barcode: cd.barcode || '',
        country: cd.country || '',
        format: cd.format || 'CD',
        editionNotes: cd.editionNotes || '',
        genres: Array.isArray(cd.genres) ? cd.genres : (cd.genres ? [cd.genres as string] : []),
        recordingQuality: cd.recordingQuality || '',
        producer: Array.isArray(cd.producer) ? cd.producer : (cd.producer ? [cd.producer as string] : []),
        engineer: Array.isArray(cd.engineer) ? cd.engineer : (cd.engineer ? [cd.engineer as string] : []),
  ...
  const handleArrayInputChange = (field: string, value: string) => {
    const array = value.split(',').map((item: string) => item.trim()).filter((item: string) => item);
  ...
  const handleArtistInputBlur = (value: string) => {
  ...
  const handleOwnershipChange = (field: string, value: string) => {
    setFormData(prev => ({
      ...prev,
      ownership: {
        ...prev.ownership,
  ...
  const deleteDisc = (discIndex: number) => {
    setFormData(prev => ({
      ...prev,
      discs: prev.discs.filter((_: DiscData, idx: number) => idx !== discIndex).map((disc: DiscData, idx: number) => ({
        ...disc,
        number: idx + 1
      }))
    }));
  ...
  const editTrack = (discIndex: number, trackIndex: number) => {
  ...
  const saveTrack = () => {
    ...
    setFormData(prev => {
      ...
      if (trackIndex === -1) {
        ...
        disc.tracks = disc.tracks.map((t: TrackData, idx: number) => idx === trackIndex ? data : t);
      }

      // Renumber all tracks based on position
      disc.tracks = disc.tracks.map((track: TrackData, idx: number) => ({
        ...track,
        trackNumber: idx + 1
      }));
  ...
  const deleteTrack = (discIndex: number, trackIndex: number) => {
    setFormData(prev => {
      const newDiscs = [...prev.discs];
      const disc = { ...newDiscs[discIndex] };
      disc.tracks = disc.tracks
        .filter((_: TrackData, idx: number) => idx !== trackIndex)
        .map((track: TrackData, idx: number) => ({ ...track, trackNumber: idx + 1 }));
      newDiscs[discIndex] = disc;
      return { ...prev, discs: newDiscs };
    });
  };
  ...
  const parseDurationPartial = (timeString: string) => {
    if (!timeString) return null;
    const parts = timeString.split(':');
    if (parts.length === 1) {
  ...
  const handleTrackDragStart = (e: React.DragEvent<HTMLTableRowElement>, discIndex: number, trackIndex: number) => {
  ...
  const handleTrackDragEnd = (e: React.DragEvent<HTMLTableRowElement>) => {
    (e.currentTarget as HTMLElement).style.opacity = '1';
    setDraggedTrack(null);
    setDropTarget(null);
  };

  const handleTrackDragOver = (e: React.DragEvent<HTMLTableRowElement>, discIndex: number, trackIndex: number) => {
    e.preventDefault();
  ...
  const handleTrackDrop = (e: React.DragEvent<HTMLTableRowElement>, targetDiscIndex: number, targetTrackIndex: number) => {
    e.preventDefault();
    ...
    setFormData(prev => {
      ...
      disc.tracks = tracks.map((track: TrackData, idx: number) => ({
        ...track,
        trackNumber: idx + 1
      }));
  ...
  const validateForm = () => {
    ...
    if (formData.releaseYear && (isNaN(Number(formData.releaseYear)) || Number(formData.releaseYear) < 1900 || Number(formData.releaseYear) > new Date().getFullYear() + 1)) {
      newErrors.releaseYear = 'Please enter a valid year';
    }

    if (formData.ownership.priceChf && (isNaN(Number(formData.ownership.priceChf)) || Number(formData.ownership.priceChf) < 0)) {
  ...
  const getArrayDisplayValue = (array: string[]) => {
    return Array.isArray(array) ? array.join(', ') : '';
  };

  /**
   * A photograph waits here while its corners are confirmed.
   *
  ...
  const uploadCoverFile = async (file: File) => {
    ...
    try {
      ...
      setUploadMessageType('danger');
      setUploadMessage('Failed to upload cover: ' + (error as Error).message);
    } finally {
  ...
  const uploadBackCoverFile = async (file: File) => {
    ...
    try {
      ...
      setUploadMessageType('danger');
      setUploadMessage('Failed to upload back cover: ' + (error as Error).message);
    } finally {
      setUploadingCover(false);
    }
  };
        ...
        <Modal.Body>
          ...
          {availableCovers.length > 1 && cd && !cd.id && (
            <Form.Group className="mb-4">
              ...
              <div style={{
                ...
                {availableCovers.map((cover: AvailableCover, index: number) => (
                  <div
                    key={index}
                    onClick={() => {
            ...
            <Col md={6}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleArtistInputChange(e.target.value)}
                  onBlur={(e: React.FocusEvent<HTMLInputElement>) => handleArtistInputBlur(e.target.value)}
                ...
                >
                  ...
                  />
                      {getCoverImageUrl('front') ? (
                        <>
                          <img
                            src={getCoverImageUrl('front') ?? undefined}
                            alt="Album Cover"
                            style={{
                              width: '100%',
                              height: '100%',
              ...
              <Form.Group>
                ...
                >
                  ...
                  {getCoverImageUrl('back') ? (
                    <>
                      <img
                        src={getCoverImageUrl('back') ?? undefined}
                        alt="Back Cover"
                        style={{
                          width: '100%',
                          height: '100%',
            ...
            <Col md={3}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleInputChange('releaseYear', e.target.value)}
            ...
            <Col md={3}>
              <Form.Group className="mb-3">
                ...
                <Form.Select
                  ...
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleInputChange('format', e.target.value)}
                >
            ...
            <Col md={3}>
              <Form.Group className="mb-3">
                ...
                <Form.Select
                  ...
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleInputChange('recordingQuality', e.target.value)}
                >
            ...
            <Col md={4}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleArrayInputChange('labels', e.target.value)}
            ...
            <Col md={4}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleInputChange('catalogNumber', e.target.value)}
                  placeholder="[none]"
                />
              </Form.Group>
            </Col>
            ...
            <Col md={12}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleInputChange('editionNotes', e.target.value)}
            ...
            <Col md={12}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleArrayInputChange('genres', e.target.value)}
            ...
            <Col md={6}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleArrayInputChange('producer', e.target.value)}
            ...
            <Col md={6}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleArrayInputChange('engineer', e.target.value)}
            ...
            <Col md={4}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleInputChange('recordingLocation', e.target.value)}
            ...
            <Col md={4}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleInputChange('language', e.target.value)}
                  placeholder="e.g., eng"
                  maxLength={3}
                />
              </Form.Group>
            ...
            <Col md={4}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleInputChange('annotation', e.target.value)}
            ...
            <Col md={3}>
              <Form.Group className="mb-3">
                ...
                <Form.Select
                  ...
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleOwnershipChange('condition', e.target.value)}
                >
                  <option value="">-</option>
            ...
            <Col md={3}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleOwnershipChange('purchasedAt', e.target.value)}
                />
              </Form.Group>
            </Col>
            ...
            <Col md={2}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleOwnershipChange('priceChf', e.target.value)}
            ...
            <Col md={4}>
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleOwnershipChange('notes', e.target.value)}
          ...
          {formData.discs.length === 0 ? (
            ...
            <Accordion defaultActiveKey="0" className="mb-3">
              {formData.discs.map((disc: DiscData, discIndex: number) => (
                    ...
                    {disc.tracks.length === 0 ? (
                      ...
                      <>
                        ...
                        <Table striped bordered hover size="sm" style={{ fontSize: '0.875rem' }}>
                          ...
                          <tbody>
                            {disc.tracks.map((track: TrackData, trackIndex: number) => {
