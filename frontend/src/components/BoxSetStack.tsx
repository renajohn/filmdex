...
interface BoxSetStackProps {
  ...
  sortedMovies?: Movie[];
  onClose?: (e: any) => void;
  watchNextMovies?: Movie[];
  onWatchNextToggle?: (e: React.MouseEvent, movie: Movie) => void;
  dataFirstLetter?: string;
  totalCount?: number;
}

const BoxSetStack: React.FC<BoxSetStackProps> = ({ boxSetName, movies, onMovieClick, isExpanded, onToggleExpanded, sortedMovies, onClose, watchNextMovies, onWatchNextToggle, dataFirstLetter, totalCount }) => {
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if ((e.target as HTMLElement).closest('.movie-thumbnail-menu')) {
      return;
    }
    if (onToggleExpanded) {
      onToggleExpanded();
  ...
  const getCombinedScore = (movie: Movie) => {
    ...
    if (movie.rotten_tomato_rating && movie.rotten_tomato_rating !== 'N/A') {
      const rtScore = parseFloat(String(movie.rotten_tomato_rating)) / 10;
  ...
  // Get rating color
  const getRatingColor = (rating: number | string | null | undefined, maxRating: number = 10) => {
    if (!rating || rating === 'N/A') return '#3a3a3a';

    const percentage = (parseFloat(String(rating)) / maxRating) * 100;
