...
interface FillBackCoversModalProps {
  isOpen: boolean;
  onClose: () => void;
  onFill: (action: string, albumIdsOrType?: number[] | string, coverType?: string) => Promise<{ albums?: Album[]; processed?: number; successful?: number; failed?: number; errors?: string[] }>;
}

const FillBackCoversModal = ({ isOpen, onClose, onFill }: FillBackCoversModalProps) => {
  const [albums, setAlbums] = useState<Album[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [isRunning, setIsRunning] = useState<boolean>(false);
  const [result, setResult] = useState<FillResult | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [progress, setProgress] = useState<Progress>({ current: 0, total: 0, currentAlbum: '', phase: '' });
  ...
  const handleFillCovers = async () => {
    ...
    try {
      ...
      for (let i = 0; i < albums.length; i++) {
        ...
        if (!album.cover || album.cover.trim() === '') {
          try {
            ...
            results.processed += frontResult.processed || 0;
            results.successful += frontResult.successful || 0;
            results.failed += frontResult.failed || 0;
            if (frontResult.errors) {
              results.errors!.push(...frontResult.errors);
            }
          } catch (err) {
            results.failed++;
            results.errors!.push(`Front cover for ${album.title}: ${(err as Error).message}`);
          }
        } else {
      ...
      for (let i = 0; i < albums.length; i++) {
        ...
        if (!album.backCover || album.backCover.trim() === '') {
          try {
            ...
            results.processed += backResult.processed || 0;
            results.successful += backResult.successful || 0;
            results.failed += backResult.failed || 0;
            if (backResult.errors) {
              results.errors!.push(...backResult.errors);
            }
          } catch (err) {
            results.failed++;
            results.errors!.push(`Back cover for ${album.title}: ${(err as Error).message}`);
          }
        } else {
      ...
      setError((err as Error).message || 'Failed to fill covers');
    } finally {
      setIsRunning(false);
    }
  };
  ...
  const getArtistDisplay = (artist: string | string[] | undefined): string => {
  ...
  return (
    <div className="fill-back-covers-modal-overlay" onClick={handleClose}>
      <div className="fill-back-covers-modal" onClick={(e: React.MouseEvent) => e.stopPropagation()}>
