...
interface CompactRatingsWidgetProps {
...
const CompactRatingsWidget = ({
  ...
  onRefresh,
  refreshing = false
}: CompactRatingsWidgetProps) => {
  const getRatingPercentage = (rating: number | null | undefined, max: number): number => {
    if (!rating) return 0;
    return Math.min((rating / max) * 100, 100);
  };

  const getRatingColor = (rating: number | null | undefined, max: number): string => {
  ...
  const formatPercentage = (rating: number | null | undefined): string => {
    if (!rating) return '-';
