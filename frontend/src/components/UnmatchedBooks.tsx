...
interface UnmatchedBooksProps {
  importId: number | null;
  onImportComplete: () => void;
  setCurrentStep: (step: string) => void;
}

const UnmatchedBooks: React.FC<UnmatchedBooksProps> = ({ importId, onImportComplete, setCurrentStep }) => {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      setError('Failed to search for books: ' + (error as Error).message);
      setSearchResults([]);
    } finally {
  ...
  const handleResolve = async () => {
    ...
    try {
      ...
      setError('Failed to resolve book: ' + (error as Error).message);
    } finally {
      setIsResolving(false);
    }
  };

  const handleIgnore = async (bookTitle: string) => {
    setIsResolving(true);
    setError(null);
    ...
    try {
      ...
      setError('Failed to ignore book: ' + (error as Error).message);
    } finally {
      setIsResolving(false);
    }
  };
