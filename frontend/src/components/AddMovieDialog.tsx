...
interface AddMovieDialogProps {
  ...
  onMovieAdded?: (message: string, variant: string) => void;
}

let searchTimeout: ReturnType<typeof setTimeout> | null = null;

const AddMovieDialog = ({ isOpen, onClose, initialMode = 'collection', onSuccess, onMovieAdded }: AddMovieDialogProps) => {
  ...
  const handleScanCover = async () => {
    ...
    try {
      const result = await apiService.scanMovieCover(coverImage, coverMimeType as string) as ScanResult;
      ...
      if (apiErr.status === 503) {
        ...
        setScanError(apiErr.data?.error || apiErr.message || 'Failed to scan cover. Try searching by title instead.');
      }
    } finally {
      setScanning(false);
    }
  ...
  const searchMovies = async (query: string) => {
    ...
    try {
      const results = await apiService.searchAllTMDB(query) as any[];

      // Check editions for each movie
      const resultsWithStatus = await Promise.all(
        results.map(async (movie: any) => {
          try {
            const editionsCheck = await apiService.checkMovieEditions(movie.id) as any;
            return {
              ...movie,
  ...
  const handlePosterClick = () => {
    if (selectedMovie?.id) {
      if (posterRef.current) {
        ...
        const dialog = posterRef.current.closest('.add-movie-dialog') as HTMLElement | null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedMovie && selectedMovie.existingEditions) {
      const duplicate = selectedMovie.existingEditions.find(
        (edition: MovieEdition) => edition.title === trimmedTitle && edition.format === formData.format
      );
    ...
    try {
      ...
      const result = await apiService.addMovie(movieData as any);
      ...
      if (apiErr.status === 409 && apiErr.code === 'DUPLICATE_EDITION') {
        ...
        errorMessage = apiErr.data?.error || apiErr.message || 'This movie already exists';
    ...
    <div className="add-movie-dialog-overlay" onClick={handleClose}>
      ...
      <div className="add-movie-dialog" onClick={(e) => e.stopPropagation()} onKeyDown={handleKeyDown}>
        ...
        <InlinePosterSelector
          movie={selectedMovie as any ?? { id: 0 }}
          isOpen={showPosterSelector}
          onSelectPoster={handlePosterSelect as any}
              ...
              <div className="form-group">
                ...
                {searchResults.length > 0 && (
                  <div className="search-results">
                    {searchResults.map((movie: MovieResult, index: number) => {
                        const hasEditions = movie.hasEditions && movie.existingEditions && movie.existingEditions.length > 0;
                            ...
                            <div className="search-result-info">
                              ...
                              {hasEditions && (
                                <div className="existing-editions-info">
                                  ...
                                  {movie.existingEditions!.map((edition: MovieEdition, idx: number) => (
            ...
            <div className="step-content">
              ...
              {selectedMovie && (
                <div className="selected-movie-info">
                  ...
                  {selectedMovie.hasEditions && selectedMovie.existingEditions && selectedMovie.existingEditions.length > 0 && (
                    <div className="existing-editions-notice">
                      <strong>You already have this title:</strong>
                      <ul className="editions-list">
                        {selectedMovie.existingEditions.map((edition: MovieEdition, idx: number) => {
