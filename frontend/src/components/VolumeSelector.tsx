...
interface VolumeSelectorProps {
  ...
  onHide: () => void;
  seriesName: string;
  onVolumesSelected?: (volumes: Volume[]) => Promise<void>;
  templateBook?: TemplateBook;
}

const VolumeSelector = ({ show, onHide, seriesName, onVolumesSelected, templateBook }: VolumeSelectorProps) => {
  const [volumes, setVolumes] = useState<Volume[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string>('');
  const [selectedVolumes, setSelectedVolumes] = useState<Set<number>>(new Set());
  const [enriching, setEnriching] = useState<boolean>(false);
  const [enrichmentProgress, setEnrichmentProgress] = useState<{ current: number; total: number }>({ current: 0, total: 0 });
  const [existingBooks, setExistingBooks] = useState<Map<number, ExistingBook>>(new Map()); // Map of seriesNumber -> book
  const [copyOwnerInfo, setCopyOwnerInfo] = useState<boolean>(true);
  ...
  }, [show, seriesName, templateBook]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const loadVolumes = async () => {
    ...
    try {
      ...
      const [results, existing] = await Promise.all([
        bookService.searchSeriesVolumes(seriesName, { maxVolumes: 50, language: normalizedLanguage }) as Promise<Volume[]>,
        bookService.getBooksBySeries(seriesName).catch(() => [] as ExistingBook[]) as Promise<ExistingBook[]>
      ]);

      // Create a map of existing books by series number
      const existingMap = new Map<number, ExistingBook>();
      existing.forEach((book: ExistingBook) => {
      ...
      if (templateBook && templateBook.seriesNumber) {
        ...
        const toSelect = results
          ...
          .map((v: Volume) => v.seriesNumber);
        setSelectedVolumes(new Set(toSelect));
      }
    } catch (err) {
      setError('Failed to load volumes: ' + (err as Error).message);
    } finally {
      setLoading(false);
    }
  };

  const toggleVolume = (seriesNumber: number) => {
  ...
  const handleAddSelected = async () => {
    ...
    try {
      ...
      const volumesToAdd = volumes.filter((v: Volume) => {
      ...
      for (let i = 0; i < volumesToAdd.length; i += BATCH_SIZE) {
        ...
        const batchResults = await Promise.allSettled(
          batch.map(async (volume: Volume) => {
            try {
              const enriched = await bookService.enrichBook(volume as unknown as Record<string, unknown>) as Volume;
              ...
              const volumeWithOwner: Volume = { ...volume };
        ...
        batchResults.forEach((result) => {
          if (result.status === 'fulfilled') {
            ...
            if (volumeIndex < volumesToAdd.length) {
              const volumeWithOwner: Volume = { ...volumesToAdd[volumeIndex] };
              if (copyOwnerInfo && templateBook) {
                if (templateBook.owner !== undefined) {
                  volumeWithOwner.owner = templateBook.owner || null;
                }
      ...
      setError('Failed to process volumes: ' + (err as Error).message);
    } finally {
      setEnriching(false);
  ...
  const getCoverImage = (volume: Volume): string | null => {
            ...
            {templateBook && (
              <div className="mb-3">
                <Form.Check
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCopyOwnerInfo(e.target.checked)}
                  disabled={enriching}
                />
              </div>
            )}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="volume-info">
                      ...
                      {volume.authors && (Array.isArray(volume.authors) ? volume.authors.length > 0 : volume.authors) && (
