...
interface ReleaseCoverMapEntry {
  front: CoverMeta | null;
  back: CoverMeta | null;
}

interface ModalCoverData {
  url: string;
  title?: string;
  artist?: string | string[];
  coverType: string;
...
const AlbumMetadataForm: React.FC<AlbumMetadataFormProps> = ({
  show,
  onHide,
  release,
  allReleasesInGroup = [],
  ...
  const [selectedFrontCoverIndex, setSelectedFrontCoverIndex] = useState<number | null>(null);
  const [selectedBackCoverIndex, setSelectedBackCoverIndex] = useState<number | null>(null);
  const [customCoverPreview, setCustomCoverPreview] = useState<string | null>(null);
  const [customBackCoverPreview, setCustomBackCoverPreview] = useState<string | null>(null);
  const [showCoverModal, setShowCoverModal] = useState(false);
  const [modalCoverData, setModalCoverData] = useState<ModalCoverData | null>(null);
  const [imageDimensions, setImageDimensions] = useState<Record<string, { width: number; height: number }>>({});
  const [sortedCovers, setSortedCovers] = useState<CoverArt[]>([]);
  const [releaseCoverMap, setReleaseCoverMap] = useState<Record<string, ReleaseCoverMapEntry>>({}); // { releaseId: { front: {display, full, sizes}, back: {...} } }
    ...
    group.forEach(r => {
      ...
      if (meta?.front) {
        covers.push({
          ...
          fullUrl: meta.front.full || undefined,
      ...
      if (meta?.back) {
        covers.push({
          ...
          fullUrl: meta.back.full || undefined,
  ...
  useEffect(() => {
    const fetchCovers = async () => {
      try {
        ...
        const nextMap: Record<string, ReleaseCoverMapEntry> = { ...releaseCoverMap };
        results.forEach((res, idx) => {
          const id = missing[idx];
          if (res.status === 'fulfilled' && res.value) {
            const frontMeta = (res.value as any)?.front;
            const backMeta = (res.value as any)?.back;
            const front: CoverMeta | null = frontMeta ? {
            ...
            const back: CoverMeta | null = backMeta ? {
  ...
  const formatImageSize = (width: number, height: number) => {
  ...
  const handleFrontCoverSelect = (index: number) => {
    setSelectedFrontCoverIndex(index);
    setCustomCoverPreview(null);
  };

  const handleBackCoverSelect = (index: number) => {
    setSelectedBackCoverIndex(index);
    setCustomBackCoverPreview(null);
  };

  const handleCoverClick = (cover: CoverArt, coverType: string) => {
    setModalCoverData({
  ...
  const handleCustomCoverUpload = async (event: React.ChangeEvent<HTMLInputElement>, type: string = 'front') => {
    ...
    try {
      ...
      reader.onload = (e) => {
        if (type === 'front') {
          ...
          setCustomBackCoverPreview(e.target?.result as string);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (selectedFrontCoverIndex !== null && selectedFrontCoverIndex >= 0 && sortedCovers[selectedFrontCoverIndex]) {
      ...
      } else if (selectedBackCoverIndex !== null && selectedBackCoverIndex >= 0 && sortedCovers[selectedBackCoverIndex]) {
      ...
      setUploadMessage('Failed to add album: ' + (error as Error).message);
      setUploadMessageType('danger');
      try {
        if (onAddError) onAddError(error as Error);
      } catch (_) {}
    } finally {
                        ...
                        return (
                          ...
                          >
                            <Image
                              ...
                              onLoad={(e: React.SyntheticEvent<HTMLImageElement>) => { (e.target as HTMLImageElement).classList.remove('loading'); }}
                            />
