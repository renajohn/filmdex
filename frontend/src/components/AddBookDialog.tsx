...
interface BookData {
  ...
  readDate?: string;
  _openLibraryData?: { workKey?: string; [key: string]: any };
  [key: string]: any;
}
...
interface AddBookDialogProps {
  ...
  templateBook?: BookData;
  onAddBooksBatch?: (books: any[]) => Promise<void>;
  defaultTitleStatus?: string;
  onShowAlert?: (message: string, variant: string) => void;
}

const AddBookDialog = ({ show, onHide, onAddBook, onAddStart, onBookAdded, onAddError, templateBook, onAddBooksBatch, defaultTitleStatus, onShowAlert }: AddBookDialogProps) => {
  ...
  const [groupedResults, setGroupedResults] = useState<BookGroup[]>([]);
  ...
  // Cover scan state
  const [coverImage, setCoverImage] = useState<string | null>(null); // base64
  const [coverPreview, setCoverPreview] = useState<string | null>(null); // data URL for preview
  const [coverMimeType, setCoverMimeType] = useState<string | null>(null);
  const [scanning, setScanning] = useState(false);
  const [scanResult, setScanResult] = useState<ScanResult | null>(null);
  const [scanError, setScanError] = useState<string | null>(null);
  const [coverScanAvailable, setCoverScanAvailable] = useState<boolean | null>(null); // null = unknown, true/false
  ...
  const detectBookType = (isbn: string | undefined, genres: any[] = []) => {
  ...
  useEffect(() => {
    if (show) {
      ...
      const fetchOwners = async () => {
        try {
          const suggestions = await bookService.getAutocompleteSuggestions('owner', '') as any[];
          const owners = suggestions
            .map((s: any) => typeof s === 'string' ? s : (s.owner || s.value || s))
            .filter(Boolean)
  ...
  useEffect(() => {
    if (show && coverScanAvailable === null) {
      apiService.checkCoverScanHealth()
        .then(health => setCoverScanAvailable((health as any).available))
  ...
  const handleVolumesSelected = async (enrichedVolumes: any[]) => {
    if (!onAddBooksBatch) {
  ...
  const groupSearchResults = (results: BookData[]): BookGroup[] => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const results = await bookService.searchExternalBooks(query, filters) as BookData[];
  ...
  const handleSelectBook = async (book: BookData, allBooksInGroup: BookData[] | null = null, bookIndex: number | null = null, groupIndex: number | null = null, listIndex: number | null = null) => {
    ...
    try {
      const enrichedBook = await bookService.enrichBook(book as any) as BookData;
      ...
      if (allBooksInGroup && allBooksInGroup.length > 1) {
        const enrichedGroup = await Promise.all(
          allBooksInGroup.map(b => bookService.enrichBook(b as any) as Promise<BookData>)
        );
  ...
  const handleQuickAdd = async (book: BookData) => {
    ...
    try {
      ...
      let enrichedBook: BookData;
      try {
        enrichedBook = await bookService.enrichBook(book as any) as BookData;
      } catch (enrichError) {
      ...
      if (addMultipleBooks) {
        ...
        const quickGenres = enrichedBook.genres || book.genres || [];
        const detectedType = detectBookType(isbn13, quickGenres);
        if (detectedType !== 'book') {
          setQuickAddBookType(detectedType);
        }
      ...
      setError(`Failed to add book: ${(err as any).message || 'Unknown error'}`);
      if (onAddError) {
        onAddError(err);
      }
    } finally {
  ...
  const handleFileSelect = useCallback((file: File | undefined | null) => {
    ...
    const extMimeMap: Record<string, string> = { heic: 'image/heic', heif: 'image/heif' };
    const mimeType = file.type || (ext ? extMimeMap[ext] : undefined);
  ...
  const handleScanCover = async () => {
    ...
    try {
      const result = await apiService.scanBookCover(coverImage, coverMimeType as string) as ScanResult;
      ...
      if (result.candidates && result.candidates.length > 0) {
        ...
        if (result.confidence === 'high') {
          ...
          setEnriching(true);
          try {
            const enrichedBook = await bookService.enrichBook(bestMatch as any) as BookData;
            setSelectedBook(enrichedBook);
          ...
          const enrichedCandidates = await Promise.all(
            result.candidates.map(async (c: BookData) => {
              try {
                return await bookService.enrichBook(c as any) as BookData;
              } catch (err) {
                return c;
              }
            })
      ...
      if (apiErr.status === 503) {
        ...
        setScanError(apiErr.data?.error || apiErr.message || 'Failed to scan cover. Try searching by ISBN or title.');
      }
    } finally {
      setScanning(false);
    }
  ...
  const handleShowScanCandidates = () => {
    ...
    Promise.all(
      candidates.map(async (c: BookData) => {
        try { return await bookService.enrichBook(c as any) as BookData; } catch { return c; }
      })
    ).then(enriched => {
  ...
  const formatIsbnProgressive = (digits: string) => {
    ...
    if (isDeleting && isbnInputRef.current) {
      setTimeout(() => {
        if (isbnInputRef.current) {
          ...
          const digitsBeforeCursor = inputValue.substring(0, cursorPosition || 0).replace(/\D/g, '').length;
  ...
  const handleIsbnBlur = (e: React.FocusEvent<HTMLInputElement>) => {
    const inputValue = e.target.value;
  ...
  const handleIsbnPaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    e.preventDefault();
  ...
  const getLanguageBadgeVariant = (lang: string | undefined) => {
  ...
  if (showVolumeSelector && templateBook && templateBook.series) {
    return (
      <VolumeSelector
        ...
        templateBook={templateBook as any}
  ...
  if (showMetadataForm) {
    ...
    return (
      <BookForm
        ...
        onShowOtherResults={scanResult?.candidates && scanResult.candidates.length > 1 ? handleShowScanCandidates : undefined}
      ...
      <Modal.Body className="add-book-dialog-body">
        ...
        {!showCoverScanStep && (
            ...
            <div className="row g-3 mb-3">
              ...
              <div className="col-12 col-md-4">
                <Form.Control
                  ...
                  value={searchAuthor}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSearchAuthor(e.target.value)}
                  onKeyPress={handleKeyPress}
                  className="search-input"
                />
              </div>
          ...
          {!searching && groupedResults.length > 0 && (
            <div className="grouped-search-results">
              ...
              {groupedResults.map((group: BookGroup, groupIndex: number) => {
                            ...
                            <div className="quick-add-options-inline">
                              <div className="quick-add-field owner-field">
                                ...
                                <div className="owner-autocomplete">
                                  <Form.Control
                                    ...
                                    type="text"
                                    value={quickAddOwner}
                                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleOwnerInputChange(e.target.value)}
                                    onFocus={() => {
                                      setFilteredOwners(availableOwners);
                                      setShowOwnerDropdown(availableOwners.length > 0);
                                    }}
                              ...
                              <div className="quick-add-field">
                                ...
                                <Form.Select
                                  ...
                                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setQuickAddTitleStatus(e.target.value)}
                                  onClick={(e: React.MouseEvent) => e.stopPropagation()}
                                >
                      ...
                      {hasMultipleEditions && isExpanded && (
                        <div className="editions-table-container">
                          <Table className="editions-table" hover>
                            ...
                            <tbody>
                              {group.books.map((book: BookData, bookIndex: number) => {
                                // Determine source
        ...
        <Modal.Footer className="add-book-dialog-footer">
          <Form.Check
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAddMultipleBooks(e.target.checked)}
            className="me-auto"
          />
