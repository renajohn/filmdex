...
interface ImportStats {
  ...
  autoResolvedBooks?: number;
  manualResolvedBooks?: number;
}

const BookImportPage: React.FC = () => {
  ...
  const handleFileUpload = async (file: File) => {
    try {
      setError(null);
      const response = await apiService.uploadCsv(file) as { headers: string[]; filePath: string };
  ...
  const handleMappingConfirm = async (columnMapping: Record<string, string>) => {
    try {
      setShowMappingModal(false);
      setCurrentStep('processing');
      setError(null);

      const response = await apiService.processCsv(filePath as string, columnMapping) as { importId: number };
