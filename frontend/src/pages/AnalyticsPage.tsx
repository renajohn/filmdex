...
const TimelineChart: React.FC<TimelineChartProps> = ({ data, valueKey, label, color = 'var(--accent-gold)', height = 120, valuePrefix = '' }) => {
  ...
  return (
    <div className="timeline-chart" style={{ '--chart-height': `${height}px` } as React.CSSProperties}>
...
const HorizontalBars: React.FC<HorizontalBarsProps> = ({ data, nameKey, valueKey, colors, max, onClick, limit = 8, showAll = false, valueFormat }) => {
  const [hovered, setHovered] = useState<number | null>(null);
  ...
  const isTruncated = (name: any) => String(name).length > 16;
...
const PercentileHistogram: React.FC<PercentileHistogramProps> = ({ data, percentiles }) => {
  ...
  const getPercentilePosition = (pValue: number | undefined) => {
...
const StatCard: React.FC<StatCardProps> = ({ icon: Icon, value, label, accent = 'gold' }) => (
...
const RankedList: React.FC<RankedListProps> = ({ items, nameKey, valueKey, valueFormat, subKey, onClick, limit = 8, colors, icon: Icon }) => {
  const [hovered, setHovered] = useState<number | null>(null);
...
// ============= FORMATTERS =============
const fmt = {
  currency: (v: any) => `CHF ${Number(v || 0).toFixed(0)}`,
  duration: (mins: any) => {
  ...
  percent: (n: any) => `${Number(n || 0).toFixed(0)}%`,
  rating: (n: any) => Number(n || 0).toFixed(1),
};
...
const AnalyticsPage: React.FC = () => {
  ...
  // Navigation helpers
  const goFilm = useCallback((q: string) => navigate(`/filmdex?search=${encodeURIComponent(q)}`), [navigate]);
  const goMusic = useCallback((q: string) => navigate(`/musicdex?search=${encodeURIComponent(q)}`), [navigate]);
  const goBook = useCallback((q: string) => navigate(`/bookdex?search=${encodeURIComponent(q)}`), [navigate]);
    ...
    const load = async () => {
      try {
        ...
        const [film, music, book] = await Promise.all([
          apiService.getAnalytics() as Promise<AnalyticsResponse>,
          apiService.getMusicAnalytics() as Promise<AnalyticsResponse>,
          apiService.getBookAnalytics() as Promise<AnalyticsResponse>,
        ]);
            ...
            {(film.mostWatchedMovies || []).length > 0 && (
              <Card title="Most Watched" icon={BsPlayCircle} className="featured-card accent-teal">
                <RankedList
                  ...
                  subKey="genre"
                  valueFormat={(v: any) => `${v}×`}
                  onClick={(d: any) => goFilm(`title:"${d.title}"`)}
                  colors={PALETTE.teal}
                  limit={6}
                />
              </Card>
            ...
            {(film.lastWatchedMovies || []).length > 0 && (
              <Card title="Recently Watched" icon={BsEye} className="featured-card accent-amber">
                <RankedList
                  ...
                  subKey="genre"
                  valueFormat={(v: any) => v ? new Date(v).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : ''}
                  onClick={(d: any) => goFilm(`title:"${d.title}"`)}
            ...
            {(film.topRatedUnwatched || []).length > 0 && (
              <Card title="Top Rated Unwatched" icon={BsStar} className="featured-card accent-emerald">
                <RankedList
                  ...
                  valueKey="rating"
                  subKey="genre"
                  valueFormat={(v: any) => fmt.rating(v)}
                  onClick={(d: any) => goFilm(`title:"${d.title}"`)}
              ...
              <div className="card-row">
                <Card title="Top Genres" icon={BsCollection} className="card-half">
                  <HorizontalBars
                    ...
                    onClick={(d: any) => goFilm(`genre:"${d.genre}"`)}
                  />
                </Card>
                <Card title="Format" icon={BsDisc} className="card-half">
                  <HorizontalBars
                    ...
                    valueKey="count"
                    colors={PALETTE.amber}
                    onClick={(d: any) => goFilm(`format:"${d.format}"`)}
                    showAll={true}
                  />
                </Card>
              </div>
            ...
            <div className="grid-side">
              <Card title="Top Directors" icon={BsListOl}>
                <RankedList
                  ...
                  nameKey="director"
                  valueKey="count"
                  onClick={(d: any) => goFilm(`director:"${d.director}"`)}
                  colors={PALETTE.gold}
                />
              </Card>
              ...
              <Card title="Top Actors" icon={BsPeople}>
                <RankedList
                  ...
                  nameKey="actor"
                  valueKey="count"
                  onClick={(d: any) => goFilm(`actor:"${d.actor}"`)}
                  colors={PALETTE.rose}
                />
              </Card>
            ...
            <div className="grid-main">
              <Card title="Top Genres" icon={BsCollection}>
                <HorizontalBars
                  ...
                  valueKey="count"
                  colors={PALETTE.spectrum}
                  onClick={(d: any) => goMusic(`genre:"${d.genre}"`)}
                />
              </Card>
              ...
              <div className="card-row">
                <Card title="Record Labels" icon={BsDisc} className="card-half">
                  <HorizontalBars
                    ...
                    valueKey="count"
                    colors={PALETTE.spectrum}
                    onClick={(d: any) => goMusic(`label:"${d.label}"`)}
                    limit={6}
                  />
                </Card>
                <Card title="Release Countries" icon={BsGlobe} className="card-half">
                  <HorizontalBars
                    ...
                    valueKey="count"
                    colors={PALETTE.gold}
                    onClick={(d: any) => goMusic(`country:"${d.code || d.country}"`)}
                    showAll={true}
                  />
                </Card>
              </div>
            ...
            <div className="grid-side">
              ...
              <Card title="Top Artists" icon={BsPeople}>
                <RankedList
                  ...
                  nameKey="artist"
                  valueKey="count"
                  onClick={(d: any) => goMusic(`artist:"${d.artist}"`)}
                  colors={PALETTE.gold}
                />
              </Card>
              ...
              <Card title="Longest Albums" icon={BsClock}>
                <RankedList
                  ...
                  nameKey="title"
                  valueKey="duration"
                  valueFormat={(v: any) => fmt.time(v)}
                  onClick={(d: any) => goMusic(`title:"${d.title}"`)}
                  colors={PALETTE.rose}
                  limit={6}
                />
              </Card>
              ...
              {(music.trackOverlap || []).length > 0 && (
                <Card title="Shared Tracks" icon={BsLayers}>
                  <RankedList
                    ...
                    nameKey="title"
                    valueKey="albumCount"
                    valueFormat={(v: any) => `${v} albums`}
                    onClick={(d: any) => goMusic(`track:"${d.title}"`)}
                    colors={PALETTE.warm}
                    limit={6}
                  />
                </Card>
            ...
            <div className="grid-main">
              <Card title="Top Genres" icon={BsCollection}>
                <HorizontalBars
                  ...
                  valueKey="count"
                  colors={PALETTE.spectrum}
                  onClick={(d: any) => goBook(`genre:"${d.genre}"`)}
                />
              </Card>
              ...
              <div className="card-row">
                <Card title="Publishers" icon={BsDisc} className="card-half">
                  <HorizontalBars
                    ...
                    valueKey="count"
                    colors={PALETTE.spectrum}
                    onClick={(d: any) => goBook(`publisher:"${d.publisher}"`)}
                    limit={6}
                  />
                </Card>
                <Card title="Languages" icon={BsPeople} className="card-half">
                  <HorizontalBars
                    ...
                    valueKey="count"
                    colors={PALETTE.gold}
                    onClick={(d: any) => goBook(`language:"${d.language}"`)}
                    showAll={true}
                  />
                </Card>
              </div>
            ...
            <div className="grid-side">
              ...
              <Card title="Top Authors" icon={BsPeople}>
                <RankedList
                  ...
                  nameKey="author"
                  valueKey="count"
                  onClick={(d: any) => goBook(`author:"${d.author}"`)}
                  colors={PALETTE.gold}
                />
              </Card>
              ...
              <Card title="Longest Books" icon={BsLayers}>
                <RankedList
                  ...
                  nameKey="title"
                  valueKey="pages"
                  valueFormat={(v: any) => `${v}p`}
                  onClick={(d: any) => goBook(`title:"${d.title}"`)}
                  colors={PALETTE.rose}
                  limit={6}
                />
              </Card>
              ...
              {(book.seriesDistribution || []).length > 0 && (
                <Card title="Top Series" icon={BsCollection}>
                  <RankedList
                    ...
                    nameKey="series"
                    valueKey="count"
                    onClick={(d: any) => goBook(`series:"${d.series}"`)}
                    colors={PALETTE.warm}
                    limit={6}
                  />
                </Card>
