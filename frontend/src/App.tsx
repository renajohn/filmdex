...
function AppContent() {
  ...
  const movieSearchRef = useRef<any>(null);
  const wishListRef = useRef<any>(null);
  const musicDexRef = useRef<any>(null);
  const bookDexRef = useRef<any>(null);
  const searchInputRef = useRef<HTMLInputElement>(null);
  const navigate = useNavigate();
  const location = useLocation();
  // Helper to get product name from path
  const getProductFromPath = (path: string): string | null => {
  ...
  const [hasCheckedBackfill, setHasCheckedBackfill] = useState<boolean>(false);
  const [showAddMovieDialog, setShowAddMovieDialog] = useState<boolean>(false);
  const [addMovieMode, setAddMovieMode] = useState<string>('collection');
  const [showCsvExportDialog, setShowCsvExportDialog] = useState<boolean>(false);
  const [showAlbumCsvExportDialog, setShowAlbumCsvExportDialog] = useState<boolean>(false);
  ...
  const emptyFieldTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
      ...
      try {
        ...
        const response = (location.pathname === '/musicdex'
          ...
          : location.pathname === '/bookdex'
            ...
            : await apiService.getAutocompleteSuggestions(filterType, searchTerm)) as any[];

        // Extract values from response (backend now returns {field: value} format)
        let values: any[] = response.map((item: any) => item[filterType]).filter((value: any) => value && value !== 'undefined');

        // For collections, include collection type information
        let collectionTypes: Record<string, string> = {};
        if (filterType === 'collection' || filterType === 'box_set') {
          collectionTypes = response.reduce((acc: Record<string, string>, item: any) => {
        ...
        const existingValuesList = existingValues.split(',').map((v: string) => v.replace(/"/g, '').trim().toLowerCase()).filter((v: string) => v);
        values = values.filter((v: any) => !existingValuesList.includes(v.toLowerCase()));
  ...
  const fetchFilterData = async () => {
    try {
      if (location.pathname === '/bookdex') {
        ...
        try {
          const ownersData = await bookService.getAutocompleteSuggestions('owner', '') as any[];
          const ownerNames = ownersData
            .map((item: any) => item.owner || item)
        ...
        const collectionsData = await apiService.getAllCollections() as any[];
  ...
  const handleSearchFromMovieDetail = (predicate: string) => {
  ...
  const handleFilterSelection = (filterType: string, filterValue: string) => {
    let predicate = '';
  ...
  const getAutocompleteHint = (option: AutocompleteOption): string => {
    if (option.isValue) {
      ...
      const hintMap: Record<string, string> = location.pathname === '/musicdex'
        ? {
  ...
  const selectAutocompleteSuggestion = async (option: AutocompleteOption) => {
  ...
  const handleCsvExport = async (columns: string[]) => {
    try {
      ...
      handleShowAlert('Failed to export CSV: ' + (error as Error).message, 'danger');
    }
  };
  ...
  const handleAlbumCsvExport = async (columns: string[]) => {
    try {
  ...
  const handleWishListAddMovie = (mode: string) => {
    setAddMovieMode(mode);
    setShowAddMovieDialog(true);
  };

  const handleMovieAddedToWishList = (movie: any) => {
  ...
  const handleWishListAddAlbum = (mode: string) => {
  ...
  const handleWishListAddBook = (mode: string) => {
  ...
  const handleSearchFromAlbumDetail = (criteria: SearchCriteria) => {
  ...
  const handleShowAlert = (message: string, type: string) => {
  ...
  const handleMovieAdded = (message: string, type: string) => {
  ...
  const currentProductRef = useRef<string | null>(getProductFromPath(location.pathname));
