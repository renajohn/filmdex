...
import backfillService, { type BackfillResult, type RetryProgress, type RetryResult } from '../services/backfillService';
...
const backfillController = {
  ...
  startBackfill: async (req: Request, res: Response): Promise<void> => {
    try {
  ...
  // Retry failed movies
  retryFailedMovies: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const result = await backfillService.retryFailedMovies(movieIds, {
        delayMs: 1000,
        onProgress: (progress: RetryProgress) => {
          logger.info('Retry progress:', progress);
        },
        onComplete: (result: RetryResult) => {
...
export default backfillController;
