...
interface ImportResult {
  ...
  unmatchedMovies: UnmatchedMovieEntry[];
}
...
interface ImportStatus {
  ...
  totalMovies: number;
  processedMovies: number;
  unmatchedMovies: UnmatchedMovieFormatted[];
}
...
const ImportService = {
  ...
  createImportSession: async (): Promise<{ id: string; status: string; createdAt: string; updatedAt: string }> => {
    try {
  ...
  parseCsvHeaders: async (filePath: string): Promise<string[]> => {
  ...
  processCsvFileWithMapping: async (filePath: string, importId: string, columnMapping: ColumnMapping): Promise<ImportResult> => {
    try {
      // Update import status to processing
      await MovieImport.updateStatus(importId, 'PROCESSING');

      const csvData: CsvRow[] = [];
      const unmatchedMovies: UnmatchedMovieEntry[] = [];
          ...
          try {
            ...
            if (!processedMovie) {
              ...
              return {
                ...
                csvData: mappedMovie
              } as UnmatchedMovieEntry;
            }
            return null; // Successfully processed
          } catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            console.error(`Error processing movie ${csvMovie.title}:`, errorMessage);
            const unmatchedMovie = {
              import_id: importId,
  ...
  processMovie: async (csvMovie: CsvRow, importId: string): Promise<(MovieData & { id: number }) | MovieRow | null> => {
    try {
      ...
      let omdbData: { imdbID?: string | null; imdbRating?: number | null; rottenTomatoRating?: string | null } | null = null;
      try {
        omdbData = await omdbService.searchMovie(tmdbDetails.title as string, tmdbDetails.release_date ? new Date(tmdbDetails.release_date as string).getFullYear() : null);
        ...
        console.warn(`OMDB data not available for "${tmdbDetails.title}":`, message);
      }

      // Download images
      const posterPath = await imageService.downloadPoster(tmdbDetails.poster_path as string | null, tmdbDetails.id);
      const backdropPath = await imageService.downloadBackdrop(tmdbDetails.backdrop_path as string | null, tmdbDetails.id);
      ...
      const movieData: Record<string, unknown> = {
        ...
        genre: tmdbDetails.genres ? (tmdbDetails.genres as Array<{ name: string }>).map(g => g.name).join(', ') : null,
        director: tmdbDetails.credits?.crew?.find((person: { job: string }) => person.job === 'Director')?.name || null,
        cast: tmdbDetails.credits?.cast?.map((actor: { name: string }) => actor.name) || [],
      ...
      await ImportService.processCastAndCrew(createdMovie.id, tmdbDetails.credits as TMDBCredits, tmdbDetails.id);
  ...
  getUnmatchedMovies: async (importId: string): Promise<UnmatchedMovieFormatted[]> => {
    try {
  ...
  resolveMovie: async (importId: string, unmatchedMovieTitle: string, resolvedMovie: ResolvedMovie): Promise<(MovieData & { id: number }) | MovieRow> => {
    try {
      ...
      const originalCsvData = unmatchedMovie.csvData as Record<string, string>;
      ...
      const existingMovieByTmdbId = await Movie.findByTmdbId(movieData.tmdb_id as number);
      if (existingMovieByTmdbId) {
      ...
      // Check if movie already exists by title (case-insensitive)
      const existingMovieByTitle = await Movie.findByTitle(movieData.title as string);
      if (existingMovieByTitle) {
      ...
      if (movieData.imdb_id) {
        const existingMovieByImdbId = await Movie.findByImdbId(movieData.imdb_id as string);
  ...
  processCastAndCrew: async (movieId: number, credits: TMDBCredits | undefined, tmdbId: number): Promise<void> => {
    try {
      ...
      if (credits?.cast) {
        const castMembers: MovieCastData[] = [];
      ...
      if (credits?.crew) {
        ...
        if (director) {
          ...
          const crewMembers: MovieCrewData[] = [{
            movie_id: movieId,
  ...
  // Validate CSV file
  validateCsvFile: (file: MulterFile): boolean => {
    if (!file) {
