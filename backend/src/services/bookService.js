...
class BookService {
  ...
  /**
   * Format Babelio book data to our schema
   */
  formatBabelioBook(babelioData) {
    return {
      title: babelioData.title || null,
      subtitle: babelioData.subtitle || null,
      authors: babelioData.authors || null,
      isbn: babelioData.isbn || null,
      isbn13: babelioData.isbn13 || null,
      publisher: babelioData.publisher || null,
      publishedYear: babelioData.publishedYear || null,
      language: 'fr', // Babelio is French-focused
      description: babelioData.description || null,
      coverUrl: babelioData.coverUrl || null,
      pageCount: babelioData.pageCount || null,
      genres: babelioData.genres || null,
      rating: babelioData.rating || null,
      urls: babelioData.urls || null,
      _source: 'Babelio'
    };
  }
  ...
  /**
   * Search Babelio API
   * French book database
   * 
   * Note: Babelio does not provide back cover images. Only front covers are available.
   * For back covers, use OpenLibrary which provides both front and back covers.
   */
  async searchBabelio(query, limit = 20) {
    try {
      // Babelio search URL - using their search endpoint
      const searchUrl = 'https://www.babelio.com/recherche.php';
      const params = new URLSearchParams({
        Recherche: query,
        type: 'livre'
      });
      
      logger.info(`[Babelio] Searching: ${query}`);
      ...
      if (!response.data) {
        logger.warn('[Babelio] No HTML content received');
        return [];
      }
      ...
      // Babelio search results are typically in divs with class "livre_liste" or similar
      ...
      for (const selector of resultSelectors) {
        ...
        if (items.length > 0) {
          logger.info(`[Babelio] Found ${items.length} results using selector: ${selector}`);
          foundResults = true;
          ...
          items.slice(0, limit).each((index, element) => {
            try {
              ...
              const titleLink = $el.find('a[href*="/livres/"]').first();
              const title = titleLink.text().trim() || $el.find('h2, h3, .titre').first().text().trim();
              
              if (!title) {
                logger.debug(`[Babelio] Skipping result ${index} - no title found`);
                return;
              }
              
              // Extract book URL
              const bookUrl = titleLink.attr('href');
              const fullBookUrl = bookUrl && !bookUrl.startsWith('http') 
                ? `https://www.babelio.com${bookUrl}` 
                : bookUrl;
              
              // Extract author - usually near the title
              const authorLinks = $el.find('a[href*="/auteur/"]');
              const authors = [];
              authorLinks.each((i, authEl) => {
                const authorName = $(authEl).text().trim();
              ...
              if (coverUrl && !coverUrl.startsWith('http')) {
                coverUrl = coverUrl.startsWith('//') ? `https:${coverUrl}` : `https://www.babelio.com${coverUrl}`;
              }
              
              // Extract ISBN if available (often in metadata)
              const text = $el.text();
              const isbnMatch = text.match(/ISBN[:\s]*([0-9-]{10,17})/i);
              const isbn = isbnMatch ? isbnMatch[1].replace(/-/g, '') : null;
              const isbn13 = isbn && isbn.length === 13 ? isbn : null;
              ...
              // Extract description/synopsis if available
              const description = $el.find('.resume, .synopsis, .description, p').first().text().trim();
              ...
              const book = {
                ...
                rating: rating,
                urls: fullBookUrl ? { babelio: fullBookUrl } : null,
                _source: 'Babelio'
              };
              
              results.push(book);
            } catch (error) {
              logger.warn(`[Babelio] Error parsing result ${index}: ${error.message}`);
            }
          });
      ...
      if (!foundResults) {
        // Try alternative approach - look for any links to /livres/
        logger.info('[Babelio] Trying alternative parsing approach');
        const livreLinks = $('a[href*="/livres/"]');
        
        if (livreLinks.length > 0) {
          logger.info(`[Babelio] Found ${livreLinks.length} book links`);
          
          // Group by unique href to avoid duplicates
          const seenUrls = new Set();
          
          livreLinks.slice(0, limit * 2).each((index, element) => {
            try {
              const $link = $(element);
              const href = $link.attr('href');
              const fullUrl = href && !href.startsWith('http') 
                ? `https://www.babelio.com${href}` 
                : href;
              ...
              // Extract author
              const authorLink = $container.find('a[href*="/auteur/"]').first();
              ...
              logger.warn(`[Babelio] Error parsing link ${index}: ${error.message}`);
            }
          });
        }
      }
      
      logger.info(`[Babelio] Parsed ${results.length} results from HTML`);
      
      // If we have book URLs, try to fetch more details for top results
      if (results.length > 0 && results.length <= 5) {
        // For now, return what we have
        // Future: could fetch individual book pages for more details
      }
      
      return results.slice(0, limit);
    } catch (error) {
      logger.warn(`[Babelio] Search failed: ${error.message}`);
      if (error.response) {
        logger.warn(`[Babelio] Response status: ${error.response.status}`);
      }
      return [];
    }
  }
  ...
  /**
   * Enrich a book with Babelio data
   * Searches Babelio by ISBN or title/author to get additional metadata
   * Can also fetch details from a Babelio book page URL if provided
   */
  async enrichWithBabelio(book) {
    if (!book) return book;
    
    logger.info(`[Enrichment] Attempting to enrich "${book.title}" with Babelio data`);
    
    try {
      let babelioBook = null;
      
      // If we have a Babelio URL, try to fetch the book page directly
      if (book.urls?.babelio) {
        try {
          logger.info(`[Enrichment] Fetching Babelio book page: ${book.urls.babelio}`);
          const response = await axios.get(book.urls.babelio, {
            timeout: 10000,
            headers: {
              'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
              'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
          ...
          if (response.data) {
            ...
            // Extract detailed information from book page
            const title = $('h1, .titre_livre, [itemprop="name"]').first().text().trim() || book.title;
            const authors = [];
            $('a[href*="/auteur/"]').each((i, el) => {
            ...
            const isbnMatch = pageText.match(/ISBN[:\s]*([0-9-]{10,17})/i);
            const isbn = isbnMatch ? isbnMatch[1].replace(/-/g, '') : null;
            const isbn13 = isbn && isbn.length === 13 ? isbn : null;
            const isbn10 = isbn && isbn.length === 10 ? isbn : null;
            
            // Extract description
            const description = $('.resume, .synopsis, [itemprop="description"], .description').first().text().trim();
            
            // Extract cover (usually larger on book page)
            let coverUrl = $('img[src*="livres"], .couverture img').first().attr('src') || 
                          $('img[data-src*="livres"]').first().attr('data-src');
            if (coverUrl && !coverUrl.startsWith('http')) {
              coverUrl = coverUrl.startsWith('//') ? `https:${coverUrl}` : `https://www.babelio.com${coverUrl}`;
            }
            
            // Extract publisher
            const publisherMatch = pageText.match(/Éditeur[:\s]*([^\n]+)/i) || 
                                  pageText.match(/Publisher[:\s]*([^\n]+)/i);
            const publisher = publisherMatch ? publisherMatch[1].trim() : null;
            
            // Extract year
            const yearMatch = pageText.match(/(?:Paru|Published|Date)[:\s]*(?:le\s*)?(?:en\s*)?(19|20)\d{2}/i) ||
                               pageText.match(/(19|20)\d{2}/);
            const publishedYear = yearMatch ? parseInt(yearMatch[0].match(/(19|20)\d{2}/)[0]) : null;
            
            // Extract page count
            const pageMatch = pageText.match(/(\d+)\s*(?:pages?|p\.)/i);
            const pageCount = pageMatch ? parseInt(pageMatch[1]) : null;
            
            // Extract genres/themes
            const genres = [];
            $('a[href*="/themes/"], a[href*="/genres/"], .theme a').each((i, el) => {
            ...
            const ratingMatch = pageText.match(/(\d+[,.]?\d*)\s*\/\s*5/i) ||
                               $('.note, .rating').first().text().match(/(\d+[,.]?\d*)/);
            const rating = ratingMatch ? parseFloat(ratingMatch[1].replace(',', '.')) : null;
            ...
            babelioBook = {
              ...
              rating: rating,
              urls: { babelio: book.urls.babelio },
              _source: 'Babelio'
            };
            
            logger.info(`[Enrichment] ✓ Fetched Babelio book page for "${title}"`);
          }
        } catch (error) {
          logger.warn(`[Enrichment] Failed to fetch Babelio book page: ${error.message}`);
        }
      }
      
      // If no book page URL or page fetch failed, try searching
      if (!babelioBook) {
        try {
          let searchQuery = book.title;
          if (book.authors && Array.isArray(book.authors) && book.authors.length > 0) {
            searchQuery += ` ${book.authors[0]}`;
          }
          
          logger.info(`[Enrichment] Searching Babelio by title: "${searchQuery}"`);
          const searchResults = await this.searchBabelio(searchQuery, 5);
          ...
          if (searchResults && searchResults.length > 0) {
            ...
            for (const result of searchResults) {
              ...
              if (resultTitleNormalized === bookTitleNormalized || 
                  ...
                  bookTitleNormalized.includes(resultTitleNormalized)) {
                babelioBook = result;
                logger.info(`[Enrichment] ✓ Found Babelio match by title for "${book.title}"`);
                break;
              }
            }
          }
        } catch (error) {
          logger.warn(`[Enrichment] Babelio search failed: ${error.message}`);
        }
      }
      
      if (!babelioBook) {
        logger.info(`[Enrichment] No Babelio match found for "${book.title}"`);
        return book;
      }
      
      // Merge Babelio data into the book
      const enriched = { ...book };
      const enrichedFields = [];
      
      // Fill missing fields
      if (!enriched.description && babelioBook.description) {
        enriched.description = babelioBook.description;
        enrichedFields.push('description');
      }
      if (!enriched.pageCount && babelioBook.pageCount) {
        enriched.pageCount = babelioBook.pageCount;
        enrichedFields.push(`pageCount (${babelioBook.pageCount})`);
      }
      if (!enriched.publisher && babelioBook.publisher) {
        enriched.publisher = babelioBook.publisher;
        enrichedFields.push(`publisher (${babelioBook.publisher})`);
      }
      if (!enriched.publishedYear && babelioBook.publishedYear) {
        enriched.publishedYear = babelioBook.publishedYear;
        enrichedFields.push(`publishedYear (${babelioBook.publishedYear})`);
      }
      ...
      if (babelioBook.genres && Array.isArray(babelioBook.genres)) {
        babelioBook.genres.forEach(g => allGenres.add(g));
      }
      if (allGenres.size > 0) {
        enriched.genres = Array.from(allGenres);
        if (allGenres.size > (book.genres?.length || 0)) {
      ...
      // Add Babelio cover if not already present
      if (babelioBook.coverUrl) {
        if (!enriched.availableCovers) {
          enriched.availableCovers = [];
        }
        if (!enriched.availableCovers.some(c => c.url === babelioBook.coverUrl)) {
          enriched.availableCovers.push({
            source: 'Babelio',
            url: babelioBook.coverUrl,
            type: 'front'
          });
        }
        if (!enriched.coverUrl) {
          enriched.coverUrl = babelioBook.coverUrl;
          enrichedFields.push('coverUrl');
        }
      }
      ...
      if (babelioBook.urls?.babelio) {
        enriched.urls.babelio = babelioBook.urls.babelio;
        enrichedFields.push('urls');
      }
      
      if (enrichedFields.length > 0) {
        logger.info(`[Enrichment] Successfully enriched "${book.title}" with Babelio fields: ${enrichedFields.join(', ')}`);
      }
      
      return enriched;
    } catch (error) {
      logger.warn(`[Enrichment] Babelio enrichment failed: ${error.message}`);
      return book;
    }
  }

  /**
   * Enrich a single book with ALL available sources (Google Books, OpenLibrary, Babelio)
