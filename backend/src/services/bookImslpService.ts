...
interface MusicPublisher {
  name: string;
  code: string;
}

interface DetectedPublisher extends MusicPublisher {
 ...
 */
const MUSIC_PUBLISHERS: Record<string, MusicPublisher> = {
...
interface BookData {
  ...
  title?: string;
  authors?: string[] | null;
  urls?: Record<string, string> | string | null;
...
class BookImslpService {
   ...
   */
  detectMusicPublisher(ismn: string | null | undefined): DetectedPublisher | null {
    if (!ismn) return null;
   ...
   */
  async searchWork(title: string, composer: string | null): Promise<IMSLPWork | null> {
    try {
      ...
      const response: AxiosResponse<{ query?: { search?: IMSLPSearchResult[] } }> = await axios.get(searchUrl, {
      ...
      let bestMatch: IMSLPSearchResult | null = null;
      ...
      logger.warn(`[IMSLP] Search error: ${err.message}`);
      return null;
    }
  }
   ...
   */
  async getFiles(pageTitle: string): Promise<IMSLPFile[]> {
    try {
      const apiUrl = `https://imslp.org/api.php?action=parse&page=${encodeURIComponent(pageTitle)}&format=json&prop=text`;

      const response: AxiosResponse<{ parse?: { text?: { '*'?: string } } }> = await axios.get(apiUrl, {
        timeout: this.timeout,
        httpsAgent: this.httpsAgent
      });
      ...
      const err = error as { message: string };
      logger.warn(`[IMSLP] Error getting files: ${err.message}`);
      return [];
    }
  }
  ...
  async getDownloadUrl(filename: string): Promise<string | null> {
    try {
      ...
      const response: AxiosResponse<{ query?: { pages?: Record<string, ImageInfoPage> } }> = await axios.get(filePageUrl, {
        timeout: this.timeout,
        httpsAgent: this.httpsAgent
      });
      ...
      const err = error as { message: string };
      logger.warn(`[IMSLP] Error getting download URL: ${err.message}`);
      return null;
    }
  }
   ...
   */
  async findBestScore(title: string, composer: string | null = null): Promise<IMSLPScore | null> {
    try {
      ...
      const err = error as { message: string };
      logger.error(`[IMSLP] Error finding best score: ${err.message}`);
      return null;
    }
  }
   ...
   */
  async enrichMusicScore(bookData: BookData): Promise<BookData> {
    ...
    try {
      ...
      let urlsObj: Record<string, string> = {};
      if (typeof bookData.urls === 'string') {
        try {
          urlsObj = JSON.parse(bookData.urls) as Record<string, string>;
        } catch (_e) {
          urlsObj = {};
        }
      } else if (bookData.urls && typeof bookData.urls === 'object' && !Array.isArray(bookData.urls)) {
        urlsObj = bookData.urls as Record<string, string>;
      }
    ...
    } catch (imslpError: unknown) {
      const err = imslpError as { message: string };
      logger.warn(`[IMSLP] Enrichment error: ${err.message}`);
      return bookData;
    }
  }
}

export default new BookImslpService();
