...
interface PosterResult {
  success: boolean;
  posterUrl: string;
  source: 'tmdb' | 'omdb' | 'myapifilms' | 'placeholder';
  imdbId?: string;
  error?: string;
}
...
const posterService = {
  ...
  extractImdbId: (imdbUrl: string | null): string | null => {
  ...
  getPosterFromTmdb: async (imdbId: string): Promise<string | null> => {
    try {
      ...
      const findResponse: AxiosResponse<unknown> = await axios.get(
      ...
      const data = findResponse.data as TMDBFindResponse;
      ...
      console.warn('TMDb API error:', message);
      return null;
    }
  },

  // Get poster from OMDb API (fallback)
  getPosterFromOmdb: async (imdbId: string): Promise<string | null> => {
    try {
      ...
      const response: AxiosResponse<unknown> = await axios.get('http://www.omdbapi.com/', {
        params: {
          i: imdbId,
          apikey: omdbApiKey
        }
      ...
      const data = response.data as OMDBFindResponse;
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.warn('OMDb API error:', message);
      return null;
    }
  },

  // Get poster from MyAPIFilms (fallback)
  getPosterFromMyApiFilms: async (imdbId: string): Promise<string | null> => {
    try {
      ...
      if (response.status === 200 && (response.headers['content-type'] as string | undefined)?.includes('image')) {
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.warn('MyAPIFilms API error:', message);
      return null;
    }
  },

  // Generate placeholder poster URL
  getPlaceholderPoster: (title: string | null, year: string | number | null): string => {
  ...
  getMoviePoster: async (imdbUrl: string, title: string | null = null, year: string | number | null = null): Promise<PosterResult> => {
    try {
